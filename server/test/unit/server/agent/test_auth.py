# Copyright (c) 2014 Red Hat, Inc.
#
# This software is licensed to you under the GNU General Public
# License as published by the Free Software Foundation; either version
# 2 of the License (GPLv2) or (at your option) any later version.
# There is NO WARRANTY for this software, express or implied,
# including the implied warranties of MERCHANTABILITY,
# NON-INFRINGEMENT, or FITNESS FOR A PARTICULAR PURPOSE. You should
# have received a copy of GPLv2 along with this software; if not, see
# http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt.
from unittest import TestCase
from ConfigParser import ConfigParser
from StringIO import StringIO

from mock import patch, Mock
from M2Crypto import RSA, BIO
from gofer.messaging.auth import ValidationFailed

from pulp.server.agent.auth import Authenticator


RSA_KEY = """
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""

OTHER_KEY = """
-----BEGIN RSA PRIVATE KEY-----

ZdgAs+Krt4gDkn34BF5009pZf0IBANSPMeqvw4BWr3G4
-----END RSA PRIVATE KEY-----
"""

RSA_PUB = """
-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDbZu3/ml//XLdb+n8fMklr3Ckp
BYfIqYFGQzcfDyIBGJUrWZRQgwkQn2P9J8QgVyNSrByFN1DbEJvrY9Lo6kzqy8fl
h9Ws1GKMRfqJZk99/Zae/Wbn9dQqa9EYrMZR3pO5UMmpSRFzZNTWEvmP4WAP4fqu
e1VQJiUwXnMkTSO7cQIDAQAB
-----END PUBLIC KEY-----
"""

TEST_CONF = """
[authentication]
rsa_key: /etc/pki/pulp/rsa.key
[messaging]
auth_enabled: true
"""

PULP_CONF = ConfigParser()
PULP_CONF.readfp(StringIO(TEST_CONF))


class TestAuthentication(TestCase):

    @patch('pulp.server.agent.auth.pulp_conf', PULP_CONF)
    def test_enabled(self, *unused):
        authenticator = Authenticator()
        self.assertTrue(authenticator.enabled)

    @patch('pulp.server.agent.auth.pulp_conf', PULP_CONF)
    @patch('pulp.server.agent.auth.parse_bool', return_value=False)
    def test_disabled(self, *unused):
        authenticator = Authenticator()
        self.assertFalse(authenticator.enabled)

    @patch('__builtin__.open')
    @patch('pulp.server.agent.auth.pulp_conf', PULP_CONF)
    def test_get_key(self, mock_open):
        mock_fp = Mock()
        mock_fp.read = Mock(return_value=RSA_KEY)
        mock_fp.__enter__ = Mock(return_value=mock_fp)
        mock_fp.__exit__ = Mock()
        mock_open.return_value = mock_fp

        # test

        key = Authenticator.rsa_key()

        # validation

        mock_open.assert_called_with(PULP_CONF.get('authentication', 'rsa_key'))
        self.assertTrue(mock_fp.__exit__.called)
        self.assertTrue(isinstance(key, RSA.RSA))

    @patch('pulp.server.agent.auth.pulp_conf', PULP_CONF)
    @patch('pulp.server.managers.factory.consumer_manager')
    def test_get_pub_key(self, mock_factory):
        consumer_id = 'test-consumer'
        mock_manager = Mock()
        mock_manager.get_consumer = Mock(return_value={'rsa_pub': RSA_PUB})
        mock_factory.return_value = mock_manager

        # test

        key = Authenticator.rsa_pub(consumer_id)

        # validation

        self.assertTrue(isinstance(key, RSA.RSA))

    @patch('pulp.server.agent.auth.Authenticator.rsa_key')
    def test_signing(self, mock_get):
        message = 'hello'
        key = RSA.load_key_bio(BIO.MemoryBuffer(RSA_KEY))

        mock_key = Mock()
        mock_key.sign = Mock(side_effect=key.sign)
        mock_get.return_value = mock_key

        # test
        auth = Authenticator()
        signature = auth.sign(message)

        #validation
        expected = key.sign(message)

        mock_get.assert_called_with()
        mock_key.sign.assert_called_with(message)
        self.assertEqual(expected, signature)

    @patch('pulp.server.agent.auth.Authenticator.rsa_key')
    @patch('pulp.server.agent.auth.Authenticator.enabled', False)
    def test_signing_not_enabled(self, mock_get):
        authenticator = Authenticator()
        signature = authenticator.sign('hello')
        self.assertEqual(signature, '')
        self.assertFalse(mock_get.called)

    @patch('pulp.server.agent.auth.Authenticator.rsa_pub')
    def test_validated(self, mock_get):
        message = 'hello'
        consumer_id = 'test-consumer_id'
        key = RSA.load_key_bio(BIO.MemoryBuffer(RSA_KEY))

        mock_get.return_value = RSA.load_pub_key_bio(BIO.MemoryBuffer(RSA_PUB))

        # test
        signature = key.sign(message)
        authenticator = Authenticator()
        authenticator.validate(consumer_id, message, signature)

    @patch('pulp.server.agent.auth.Authenticator.rsa_pub')
    def test_not_valid(self, mock_get):
        message = 'hello'
        consumer_id = 'test-consumer_id'
        key = RSA.load_key_bio(BIO.MemoryBuffer(OTHER_KEY))

        mock_pub = Mock()
        mock_pub.verify = Mock(return_value=False)
        mock_get.return_value = mock_pub

        # test
        signature = key.sign(message)
        authenticator = Authenticator()
        self.assertRaises(ValidationFailed, authenticator.validate, consumer_id, message, signature)

    @patch('pulp.server.agent.auth.Authenticator.rsa_pub')
    def test_rsa_error_raised(self, mock_get):
        message = 'hello'
        consumer_id = 'test-consumer_id'
        key = RSA.load_key_bio(BIO.MemoryBuffer(OTHER_KEY))

        mock_get.return_value = RSA.load_pub_key_bio(BIO.MemoryBuffer(RSA_PUB))

        # test
        signature = key.sign(message)
        authenticator = Authenticator()
        self.assertRaises(ValidationFailed, authenticator.validate, consumer_id, message, signature)

    @patch('pulp.server.agent.auth.Authenticator.rsa_pub')
    @patch('pulp.server.agent.auth.Authenticator.enabled', False)
    def test_validate_not_enabled(self, mock_get):

        # test

        authenticator = Authenticator()
        authenticator.validate(None, None, None)

        # validation

        self.assertFalse(mock_get.called)